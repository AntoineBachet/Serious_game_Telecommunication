...
func _ready():
	...
	if global.get_used_item() == "source_3_shop_tiles":
		get_node("riddleOverlay").set_tip( global.get_game_text("tile_riddle3Tip") )
	else:
		_hide_tiles()
		get_node("riddleOverlay").set_tip( global.get_game_text("tile_riddle3Tip_noTiles") )
		

	global.set_state("source_3_shop_tileMoving", false)

	var riddle = global.get_riddle("tile3")
	var index_answer
	
	if !global.get_state("source_3_shop_tileRiddle3RandomNumber"):
		randomize()
		index_answer = randi()%(riddle["possibilities"].size())
		global.set_state("source_3_shop_tileRiddle3RandomNumber", str(index_answer))
	else:
		index_answer = int( global.get_state("source_3_shop_tileRiddle3RandomNumber") )
...
func _on_validate_pressed():
	if current == answer:
		...
		get_node("riddleOverlay").set_text( global.get_game_text("tile_riddle3Lose") )

func win():
	global.set_state("source_3_shop_tileRiddle3Solved", true)
	get_node("riddleOverlay").set_text( global.get_game_text("tile_riddle3Win") )
	global.give_item("source_3_shop_noodles")
